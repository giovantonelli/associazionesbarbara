class AdBlockDetectorV2 {
	constructor(t = {}) {
		this.config = {
			...
			baitElements: [{
				...
				class: "adsbygoogle",
				id: "adsense-unit-test"
			}],
			adDomains: ["googleads.g.doubleclick.net", "pagead2.googlesyndication.com", "adnxs.com", "amazon-adsystem.com"],
			totalBlock: !1 !== t.totalBlock,
			showPopup: !1 !== t.showPopup,
			preventClose: !1 !== t.preventClose,
			debugMode: t.debugMode || !1,
			messages: {
				title: t.title || "AdBlock Deve Essere Disattivato ⚠️",
				subtitle: t.subtitle || "Accesso Richiesto per Associazione Santa Barbara",
				description: t.description || this.getDefaultDescription(),
				instructions: t.instructions || "\uD83D\uDD27 Istruzioni per Continuare:",
				buttonText: t.buttonText || "Ho Disattivato AdBlock - Ricarica Pagina"
			},
			styles: {
				overlayColor: t.overlayColor || "rgba(225, 6, 0, 0.95)",
				popupBackground: t.popupBackground || "#ffffff",
				primaryColor: t.primaryColor || "#E10600",
				textColor: t.textColor || "#333333",
				maxWidth: "900px",
				maxHeight: "85vh"
			},
			contentSelectors: t.contentSelectors || ["main", ".content", ".container", "article", "section", "body > *:not(.adblock-popup-overlay)"]
		}, this.isDetected = !1, this.popupShown = !1, this.testResults = [], this.detectedType = "none", this.log = () => {}, this.init()
	}
	...
	init() {
		this.log("\uD83D\uDE80 Inizializzazione AdBlock Detector v2.0"), "loading" === document.readyState ? document.addEventListener("DOMContentLoaded", () => this.startDetection()) : this.startDetection()
	}
	async startDetection() {
		this.log("\uD83D\uDD0D Avvio rilevamento AdBlock...");
		try {
			let t = await this.runDetectionTests();
			this.handleDetectionResult(t)
		} catch (e) {
			this.log("❌ Errore rilevamento:", e), this.handleDetectionResult(!1)
		}
	}
	async runDetectionTests() {
		...
		let t = [{
				name: "Bait Elements",
				test: () => this.testBaitElements()
			}, {
				name: "Google Ads Script",
				test: () => this.testGoogleAdsScript()
			}, {
				name: "Image Blocking",
				test: () => this.testImageBlocking()
			}, {
				name: "Network Requests",
				test: () => this.testNetworkRequests()
			}],
			e = [];
		for (let i = 0; i < t.length; i++) try {
			let o = await t[i].test();
			e.push(o), this.log(`📈 ${t[i].name}: ${o?"✅ BLOCCATO":"❌ OK"}`), i < t.length - 1 && await this.sleep(100)
		} catch (a) {
			this.log(`⚠️ Errore in ${t[i].name}:`, a), e.push(!1)
		}
		this.testResults = e;
		let s = e.filter(t => t).length,
			l = s >= this.config.threshold;
		return this.log(`🎯 Risultati: ${s}/${this.config.totalTests} test positivi`), this.log(`⚖️ Soglia: ${this.config.threshold} - AdBlock ${l?"RILEVATO":"NON RILEVATO"}`), l
	}
	async testBaitElements() {
		...
		let e = 0;
		for (let i of this.config.baitElements) {
			let o = document.createElement("div");
			o.className = i.class, o.id = i.id, o.style.cssText = "width: 300px; height: 250px; background: red; display: block !important;", o.innerHTML = "Advertisement", t.appendChild(o)
		}
		await this.sleep(200);
		let a = t.querySelectorAll('[class*="ad"]');
		for (let s of a) {
			let l = window.getComputedStyle(s);
			("none" === l.display || "hidden" === l.visibility || "0" === l.opacity || 0 === s.offsetHeight || 0 === s.offsetWidth) && e++
		}
		document.body.removeChild(t);
		let n = e >= this.config.baitElements.length / 2;
		return this.log(`🎣 Elementi esca: ${e}/${this.config.baitElements.length} bloccati`), n
	}
	async testGoogleAdsScript() {
		return this.log("\uD83D\uDCDC Test: Script Google Ads..."), new Promise(t => {
			let e = document.createElement("script");
			e.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js", e.async = !0;
			let i = !1,
				o = setTimeout(() => {
					i || (i = !0, e.parentNode && e.parentNode.removeChild(e), document.querySelectorAll("ins.adsbygoogle").forEach(t => t.remove()), t(!0))
				}, 5e3);
			e.onload = () => {
				if (!i) {
					i = !0, clearTimeout(o);
					let a = void 0 !== window.adsbygoogle;
					e.parentNode && e.parentNode.removeChild(e), document.querySelectorAll("ins.adsbygoogle").forEach(t => t.remove()), t(!a)
				}
			}, e.onerror = () => {
				i || (i = !0, clearTimeout(o), e.parentNode && e.parentNode.removeChild(e), document.querySelectorAll("ins.adsbygoogle").forEach(t => t.remove()), t(!0))
			}, document.head.appendChild(e)
		})
	}
	async testImageBlocking() {
		this.log("\uD83D\uDDBC️ Test: Blocco immagini...");
		let t = ["googleads.g.doubleclick.net", "pagead2.googlesyndication.com", "googlesyndication.com"],
			e = t.map(t => new Promise(e => {
				let i = new Image,
					o = !1,
					a = setTimeout(() => {
						o || (o = !0, e(!0))
				...
				i.onload = () => {
					o || (o = !0, clearTimeout(a), e(!1))
				}, i.onerror = () => {
					o || (o = !0, clearTimeout(a), e(!0))
				}, i.src = `https://${t}/pagead/1x1.gif?_=${Date.now()}`
			})),
			i = await Promise.all(e),
			o = i.filter(t => t).length,
			a = o >= Math.ceil(t.length / 2);
		return this.log(`🖼️ Immagini: ${o}/${t.length} bloccate`), a
	}
	async testNetworkRequests() {
		this.log("\uD83C\uDF10 Test: Richieste di rete...");
		let t = ["https://www.googletagmanager.com/gtag/js?id=test", "https://googleads.g.doubleclick.net/pagead/viewthroughconversion/test", "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"],
			e = 0;
		for (let i of t) try {
			...
			"AbortError" === s.name ? e++ : (s.message.includes("blocked") || s.message.includes("Failed to fetch")) && e++
		}
		let l = e >= Math.ceil(t.length / 2);
		return this.log(`🌐 Richieste: ${e}/${t.length} bloccate`), l
	}
	handleDetectionResult(t) {
		this.isDetected = t, t ? (this.log("\uD83D\uDEAB AdBlock RILEVATO - Attivazione protezioni"), this.config.totalBlock && this.blockContent(), this.config.showPopup && this.showModal()) : (this.log("✅ AdBlock NON RILEVATO - Accesso normale"), this.allowContent())
	}
	blockContent() {
		this.log("\uD83D\uDD12 Blocco contenuto del sito"), this.config.contentSelectors.forEach(t => {
	...
	allowContent() {
		this.config.contentSelectors.forEach(t => {
			let e = document.querySelectorAll(t);
			e.forEach(t => {
	...
	showModal() {
		...
		let t = document.createElement("div");
		t.className = "adblock-popup-overlay", t.innerHTML = this.generateModalHTML(), this.injectStyles(), document.body.appendChild(t), this.popupShown = !0, this.setupModalEvents(t), setTimeout(() => t.classList.add("show"), 10)
	}
	generateModalHTML() {
		return `
	...
	setupModalEvents(t) {
		let e = t.querySelectorAll(".tab-btn"),
			i = t.querySelectorAll(".tab-panel");
		e.forEach(o => {
			o.addEventListener("click", () => {
				let a = o.dataset.tab;
				e.forEach(t => t.classList.remove("active")), o.classList.add("active"), i.forEach(t => {
					t.classList.remove("active")
				}), t.querySelector(`#tab-${a}`).classList.add("active")
			})
		}), this.config.preventClose && t.addEventListener("click", e => {
			e.target === t && (e.preventDefault(), e.stopPropagation())
		})
	}
	injectStyles() {
		if (document.getElementById("adblock-popup-styles")) return;
		let t = document.createElement("style");
		t.id = "adblock-popup-styles", t.textContent = `
...
"undefined" != typeof module && module.exports && (module.exports = AdBlockDetectorV2), window.AdBlockDetectorV2 = AdBlockDetectorV2;